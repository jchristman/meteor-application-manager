...
WMWindow.prototype.init = function() {
    ...
    this.buttons.eq(3).click(function() { self.minimize(this); return false; });
    this.buttons.eq(2).click(function() { self.restore(this); return false; });
    this.buttons.eq(1).click(function() { self.maximize(this); return false; });
    this.buttons.eq(0).click(function() { self.close(this); return false; });
...
WMWindow.prototype.updateMove = function(event) {
    ...
    if (event.pageX > parentWidth - WMWindow.EDGE_SENSITIVITY) {
        if (this.prevHeight == undefined && this.prevWidth == undefined) {
            this.prevHeight = this.moverBoxPosition.height;
            this.prevWidth = this.moverBoxPosition.width;
        }

        this.moverBoxPosition.top = 0;
        this.moverBoxPosition.left = parentWidth / 2;
        this.moverBoxPosition.height = parentHeight;
        this.moverBoxPosition.width = parentWidth / 2;
    } else if (event.pageX < WMWindow.EDGE_SENSITIVITY) {
        if (this.prevHeight == undefined && this.prevWidth == undefined) {
            this.prevHeight = this.moverBoxPosition.height;
            this.prevWidth = this.moverBoxPosition.width;
        }
        ...
        this.moverBoxPosition.top = 0;
        this.moverBoxPosition.left = 0;
        this.moverBoxPosition.height = parentHeight;
        this.moverBoxPosition.width = parentWidth / 2;
    } else if (event.pageY < WMWindow.EDGE_SENSITIVITY) {
        if (this.prevHeight == undefined && this.prevWidth == undefined) {
            this.prevHeight = this.moverBoxPosition.height;
            this.prevWidth = this.moverBoxPosition.width;
        }
        ...
        this.moverBoxPosition.left = 0;
        this.moverBoxPosition.height = parentHeight;
        this.moverBoxPosition.width = parentWidth;
    } else {
        this.moverBoxPosition.top = event.pageY - this.winOffsetY;
        this.moverBoxPosition.left = event.pageX - this.winOffsetX;
        if (this.prevHeight != undefined && this.prevWidth != undefined) {
            this.moverBoxPosition.height = this.prevHeight;
            this.moverBoxPosition.width = this.prevWidth;
...
WMWindow.prototype.endMove = function() {
    this.manager.setWindowPos(this.id(), this.translatePosition(this.moverBoxPosition));
...
WMWindow.prototype.updateResize = function(event) {
    ...
    if (event.pageY < WMWindow.EDGE_SENSITIVITY || event.pageY > parentHeight - WMWindow.EDGE_SENSITIVITY) {
        this.prevHeight = Math.abs(this.resizerBoxPosition.top - this.resizerBoxPosition.bottom) + 2;
        this.resizerBoxPosition.top = 0;
        this.resizerBoxPosition.height = parentHeight;
    } else {
        if (this.prevHeight != undefined) {
            this.resizerBoxPosition.height = this.prevHeight;
            this.prevHeight = undefined;
        } else {
...
WMWindow.prototype.endResize = function() {
    this.manager.setWindowPos(this.id(), this.translatePosition(this.resizerBoxPosition));
...
WMWindow.prototype.endMovePaneDivider = function() {
    ...
    this.manager.setPanePos(this.id(), this.dividerParentContainer.attr('id'), percentage);
...
WMWindow.prototype.minimize = function() {
    this.manager.minimizeWindow(this.id());
    this.minimized = true;
...
WMWindow.prototype.maximize = function() {
    this.manager.maximizeWindow(this.id());
    this.minimized = false;
    this.maximized = true;
}
...
WMWindow.prototype.splitPaneVertically = function(pane, percent, moveTabsToSplit) {
    ...
    this.manager.splitPaneVertically(this.id(), pane_id, percent, moveTabsToSplit);
}

WMWindow.prototype.splitPaneHorizontally = function(pane, percent, moveTabsToSplit) {
    pane_id = pane.attr('id');
    this.manager.splitPaneHorizontally(this.id(), pane_id, percent, moveTabsToSplit);
}

WMWindow.prototype.closePane = function(pane) {
    pane_id = pane.attr('id');
    this.manager.closePane(this.id(), pane_id);
}
...
WMWindow.prototype.closeAllTabs = function() {
    ...
    this.element.find('.pane-container').each(function(idx, el) {
        pane_id = $(el).attr('id');
        self.manager.closeTabsInPane(pane_id);
    });
}

WMWindow.prototype.isClosed = function() {
    return this.manager.isClosed(this.id());
}
