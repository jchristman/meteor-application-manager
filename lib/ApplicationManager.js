...
ApplicationManager.prototype.configure = function(settings) {
    var self = this;
    if (!settings.hasOwnProperty('appID'))      throw new Error("Must define an appID field in your layout. This is necessary for the database.");
    if (!settings.hasOwnProperty('open'))       settings.open = false;
    ...
    if (Meteor.isServer) {
        // Find the correct default profile
        ...
        if (default_window_profile == undefined) {
            WMCollection.insert({'default' : 'profile'});   
            default_window_profile = WMCollection.findOne({'default' : 'profile'});
            first_startup = true;
        } 

        if (default_window_profile[settings.appID] == undefined) {
            var update = {}
            update[settings.appID] = _.omit(settings, 'appID');
            
            WMCollection.update({'default' : 'profile'}, {
                $set : update
            });
        } else if (first_startup) {
            throw new Error("WARNING! THIS IS THE FIRST TIME STARTING THE APPLICATION BUT THERE MULTIPLE APPLICATIONS WITH THE ID: " + settings.appID);
        }
    }
}

ApplicationManager.prototype.processMenu = function(menu) {
    var self = this;
...
ApplicationManager.prototype.init = function(settings) {
...
ApplicationManager.prototype.getUserWindowProfile = function(user) {
    if (user == undefined)
        user = Meteor.user();
...
ApplicationManager.prototype.getOrCreateUserWindowProfile = function(user) {
...
ApplicationManager.prototype.createUserWindowProfile = function(user) {
    var default_window_profile = WMCollection.findOne({'default' : 'profile'});
...
ApplicationManager.prototype.getWindows = function() {
    var uwp = this.getUserWindowProfile();

    var openWindows = [];
    _.each(uwp, function(val, key) {
...
ApplicationManager.prototype.getTabs = function(pane_id, invert) {
    var uwp = this.getUserWindowProfile();
    var tabs;
...
ApplicationManager.prototype.getWindowById = function(uwp, window_id) {
...
ApplicationManager.prototype.getWindowByIdWithIndex = function(uwp, window_id) {
    ret = {}
...
ApplicationManager.prototype.updateWindow = function(new_window, old_window_id) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.getTabById = function(uwp, tab_id) {
...
ApplicationManager.prototype.getTabByIdWithIndex = function(uwp, tab_id) {
    ret = {};
...
ApplicationManager.prototype.getTabContent = function(tab_id) {
    var uwp = this.getUserWindowProfile();
    var tab = this.getTabById(uwp, tab_id);
    return Template[tab.template];
}

ApplicationManager.prototype.updateTab = function(new_tab) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.setTabActive = function(tab_id) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.registerWindow = function(element, data) {
    var new_window = new WMWindow(element, this, data);
    this.windows.push(new_window);
}

ApplicationManager.prototype.grabFocus = function(wm_window) {
    var self = this;
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.setWindowPos = function(wm_window, pos, minimized, maximized) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.setPanePos = function(wm_window, parent_con_id, new_percentage) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
...
ApplicationManager.prototype.setPanePosHelper = function(pane_tree, id, new_percentage) {
    if (pane_tree.id == id) {
...
ApplicationManager.prototype.splitPaneVertically = function(wm_window, pane_id, percentage, moveTabsToSplit) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
...
ApplicationManager.prototype.splitPaneHorizontally = function(wm_window, pane_id, percentage, moveTabsToSplit) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
...
ApplicationManager.prototype.splitPaneHelper = function(pane_tree, pane_id, percentage, orientation, moveTabsToSplit) {
...
ApplicationManager.prototype.closePane = function(wm_window, pane_id) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
...
ApplicationManager.prototype.closePaneHelper = function(pane_tree, pane_id, parent_tree, wm_window) {
    if (pane_tree.id == pane_id) {
...
ApplicationManager.prototype.minimizeWindow = function(wm_window) {
    if (!wm_window.maximized) wm_window.saveWindowInfo();
    this.minimize(wm_window);    
}

ApplicationManager.prototype.defaultMinimizeFunction = function(wm_window) {
...
ApplicationManager.prototype.maximizeWindow = function(wm_window) {
...
ApplicationManager.prototype.restoreWindow = function(wm_window) {
    var info = wm_window.loadWindowInfo();
    this.setWindowPos(wm_window, info, false, false);
}

ApplicationManager.prototype.isClosed = function(wm_window) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, wm_window.id());
    return win.closed;
}

ApplicationManager.prototype.closeWindow = function(wm_window) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.renameTab = function(tab_id, name) {
    var uwp = this.getUserWindowProfile();
    var tab = this.getTabById(uwp, tab_id);
    tab.title = name;
    this.updateTab(tab);
...
ApplicationManager.prototype.moveTab = function(tab_id, from_pane_id, to_pane_id) {
    var self = this;
    var uwp = this.getUserWindowProfile();
    var tab = this.getTabById(uwp, tab_id);
...
ApplicationManager.prototype.getClosedTabs = function() {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.closeTabsInPane = function(pane_id) {
    var self = this;
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.closeTab = function(tab_id) {
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.openTab = function(tab_id, dst_pane_id) {
    var self = this;
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.refreshWindow = function(window_id) {
...
ApplicationManager.prototype.getWMWindows = function() {
    var self = this;
    var uwp = this.getUserWindowProfile();
...
ApplicationManager.prototype.getWMWindowById = function(window_id) {
...
ApplicationManager.prototype.getMenu = function(window_id) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, window_id);
    return win.menubar;
}

ApplicationManager.prototype.calcZ = function(window_id) {
    var uwp = this.getUserWindowProfile();
    var win = this.getWindowById(uwp, window_id);
...
if (Meteor.isServer) {
    Accounts.validateLoginAttempt(function(attempt) {
        if (attempt.allowed) // Check for the profile and complete it before login if successful
            AppManager.getOrCreateUserWindowProfile(attempt.user);
